...
function findMissingNumber(nums: number[]): number {
  ...
  const n = nums.length; // The length of the array gives us 'n'.
  ...
  const actualSum = nums.reduce((acc, num) => acc + num, 0); // Calculate the actual sum of the numbers in the array.
  console.log(`Actual sum of the array: ${actualSum}`);
